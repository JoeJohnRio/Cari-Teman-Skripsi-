...
import com.example.cariteman.databinding.ItemPeopleAndPlaceBinding
    ...
    fun bind(response: MahasiswaHistoryDashboardResponse) {
        ...
        try {
            tvJabatanOrganisasi?.text =
                "" + response.mahasiswaTwoPkl?.pengalamanOrganisasi?.get(0)?.bidangKerja + " at " + response.mahasiswaTwoPkl?.pengalamanOrganisasi?.get(
                    0
                )?.namaOrganisasi
        }catch (e:Exception){
